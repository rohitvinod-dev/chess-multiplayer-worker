/**
 * Daily Streak Reminders Cron Job
 * Runs at 9 AM UTC daily (optimal engagement time)
 *
 * Purpose: Send training reminders to users who:
 * - Have an active streak (3+ days)
 * - Haven't trained today
 * - Have notifications enabled
 *
 * Batch size: 1000 users per run
 */

import type { FirestoreClient } from '../firestore';
import { TRIGGERS } from '../types/notifications';
import { checkPolicy } from '../utils/policy-engine';
import { sendFCM } from '../utils/fcm';

// ============ MAIN REMINDER FUNCTION ============

export async function sendDailyStreakReminders(
  firestore: FirestoreClient,
  env: { FIREBASE_PROJECT_ID: string }
): Promise<{
  success: boolean;
  sent: number;
  blocked: number;
  errors: number;
  duration: number;
}> {
  const startTime = Date.now();
  let sent = 0;
  let blocked = 0;
  let errors = 0;

  console.log('[Cron] Starting daily streak reminders...');

  try {
    // Get users with active streaks who haven't trained today
    const eligibleUsers = await getEligibleUsers(firestore);

    console.log(`[Cron] Found ${eligibleUsers.length} eligible users for streak reminders`);

    // Process in batches of 100 to avoid overwhelming FCM
    const batchSize = 100;
    for (let i = 0; i < eligibleUsers.length; i += batchSize) {
      const batch = eligibleUsers.slice(i, i + batchSize);

      const results = await Promise.all(
        batch.map((user) => sendStreakReminder(firestore, user, env))
      );

      // Count results
      for (const result of results) {
        if (result.sent) sent++;
        else if (result.blocked) blocked++;
        else if (result.error) errors++;
      }

      // Small delay between batches to avoid rate limits
      if (i + batchSize < eligibleUsers.length) {
        await delay(1000); // 1 second delay
      }
    }

    const duration = Date.now() - startTime;

    console.log(`[Cron] Daily streak reminders complete:`);
    console.log(`  - Sent: ${sent}`);
    console.log(`  - Blocked by policy: ${blocked}`);
    console.log(`  - Errors: ${errors}`);
    console.log(`  - Duration: ${duration}ms`);

    return {
      success: true,
      sent,
      blocked,
      errors,
      duration,
    };
  } catch (error) {
    console.error('[Cron] Daily reminders failed:', error);
    return {
      success: false,
      sent,
      blocked,
      errors: errors + 1,
      duration: Date.now() - startTime,
    };
  }
}

// ============ HELPER FUNCTIONS ============

interface EligibleUser {
  userId: string;
  username: string;
  streak: number;
  lastTrainingDate: string; // YYYY-MM-DD
  fcmToken?: string;
}

async function getEligibleUsers(firestore: FirestoreClient): Promise<EligibleUser[]> {
  const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
  const eligibleUsers: EligibleUser[] = [];

  try {
    // Query users with streaks >= 3
    // Note: In production, you'd want to use a Firestore index on streak field
    const users = await firestore.queryDocuments('users', [
      { field: 'currentStreak', op: 'GREATER_THAN_OR_EQUAL', value: 3 },
    ], {
      limit: 1000, // Limit to 1000 users per cron run
    });

    for (const user of users) {
      const lastTrainingDate = user.lastTrainingDate || '';

      // Check if user hasn't trained today
      if (lastTrainingDate !== today) {
        // Check if user has notifications enabled
        const prefs = await firestore.getDocument(
          `users/${user.id}/preferences/notifications`
        );

        if (prefs?.enabled !== false && prefs?.categories?.streaks !== false) {
          eligibleUsers.push({
            userId: user.id,
            username: user.username || 'Player',
            streak: user.currentStreak || 0,
            lastTrainingDate,
            fcmToken: user.latestFcmToken,
          });
        }
      }
    }
  } catch (error) {
    console.error('[Cron] Error fetching eligible users:', error);
  }

  return eligibleUsers;
}

async function sendStreakReminder(
  firestore: FirestoreClient,
  user: EligibleUser,
  env: { FIREBASE_PROJECT_ID: string }
): Promise<{ sent?: boolean; blocked?: boolean; error?: boolean }> {
  try {
    const trigger = TRIGGERS.daily_streak_reminder;

    // Load user preferences
    const userPrefs = await firestore.getDocument(
      `users/${user.userId}/preferences/notifications`
    );

    // Check notification policy
    const policyCheck = await checkPolicy(
      {
        userId: user.userId,
        trigger,
        scheduledTime: new Date(),
        userPreferences: userPrefs || { enabled: true },
      },
      firestore
    );

    if (!policyCheck.allowed) {
      console.log(
        `[Cron] Notification blocked for ${user.userId}: ${policyCheck.reason}`
      );
      return { blocked: true };
    }

    // Select random variant (A/B testing)
    const variant = trigger.variants[Math.floor(Math.random() * trigger.variants.length)];

    // Interpolate message with user data
    const title = variant.title.replace(/\{(\w+)\}/g, (_, key) => {
      return key === 'streak' ? String(user.streak) : '';
    });
    const body = variant.body.replace(/\{(\w+)\}/g, (_, key) => {
      return key === 'streak' ? String(user.streak) : '';
    });

    // Create notification ledger entry
    const notificationId = `${Date.now()}_${user.userId}_${Math.random().toString(36).substr(2, 9)}`;
    const now = { _seconds: Math.floor(Date.now() / 1000), _nanoseconds: 0 };

    await firestore.setDocument(`notification_ledger/${notificationId}`, {
      userId: user.userId,
      triggerId: trigger.id,
      category: trigger.category,
      templateId: variant.id,
      experimentVariant: variant.id,
      scheduledAt: now,
      status: 'scheduled',
      metadata: { streak: user.streak, username: user.username },
      deepLink: 'checkmatex://training',
      title,
      body,
      createdAt: now,
    });

    // Send via FCM if token available
    if (user.fcmToken) {
      const accessToken = await firestore.getAccessToken();

      const result = await sendFCM({
        projectId: env.FIREBASE_PROJECT_ID,
        accessToken,
        fcmToken: user.fcmToken,
        title,
        body,
        deepLink: 'checkmatex://training',
        trigger,
        metadata: { streak: user.streak, username: user.username },
        notificationId,
      });

      if (result.success) {
        // Update ledger with sent status
        await firestore.updateDocument(`notification_ledger/${notificationId}`, {
          status: 'sent',
          sentAt: now,
          fcmMessageId: result.messageId,
        });

        console.log(`[Cron] Sent streak reminder to ${user.userId} (streak: ${user.streak})`);
        return { sent: true };
      } else {
        // Update ledger with failed status
        await firestore.updateDocument(`notification_ledger/${notificationId}`, {
          status: 'failed',
          failureReason: result.error,
        });

        console.error(`[Cron] Failed to send to ${user.userId}: ${result.error}`);
        return { error: true };
      }
    } else {
      // No FCM token
      await firestore.updateDocument(`notification_ledger/${notificationId}`, {
        status: 'failed',
        failureReason: 'no_fcm_token',
      });

      return { error: true };
    }
  } catch (error) {
    console.error(`[Cron] Error sending reminder to ${user.userId}:`, error);
    return { error: true };
  }
}

// ============ UTILITY FUNCTIONS ============

function delay(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
